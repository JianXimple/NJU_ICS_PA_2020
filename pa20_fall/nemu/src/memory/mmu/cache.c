...
void cache_write(paddr_t paddr, size_t len, uint32_t data,CacheLine *cache)
{
	...
	for(int i=0;i<8;i++){
	    if(cache[mygroup_num*8+i].sign==mysign&&cache[mygroup_num*8+i].valid==1){
	        if(offset+len>64){
...
uint32_t cache_read(paddr_t paddr, size_t len,CacheLine *cache)
{
	...
	for(int i=0;i<8;i++){
	    if(cache[mygroup_num*8+i].valid==0){
	        cache[mygroup_num*8+i].valid=1;
	...
	cache[mygroup_num*8+change_num].valid=1;
